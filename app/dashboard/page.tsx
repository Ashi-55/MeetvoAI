...
function statusMeta(status?: string | null) {
  ...
  if (status === 'in_progress') return { text: 'In Progress', bg: '#ae9bc920', color: C.blue, border: '#ae9bc940' };
  if (status === 'submitted') return { text: 'Review Needed', bg: '#F6AD5520', color: C.warning, border: '#F6AD5540' };
  if (status === 'completed') return { text: 'Completed', bg: '#68D39120', color: C.success, border: '#68D39140' };
  if (status === 'disputed') return { text: 'Disputed', bg: '#E53E3E20', color: C.error, border: '#E53E3E40' };
  return { text: 'Open', bg: '#1E1B3A', color: C.gray, border: C.border };
}
      ...
      <main className="dash-main">
        ...
        <div className="stats-grid">
          {loading ? Array.from({ length: 4 }).map((_, i) => <div key={i} className="stat-card"><Skeleton className="h-4 w-24" /><Skeleton className="mt-5 h-8 w-20" /></div>) : (
            <>
              <StatCard label="Active Deals" value={activeDeals.length} icon={<Handshake size={18} />} iconBg="#ae9bc920" iconColor={C.blue} trend="Active right now" />
              <StatCard label="Deployed Agents" value={deployedCount} icon={<Bot size={18} />} iconBg="#7C5CBF20" iconColor={C.purple} trend="Running 24/7" />
              <StatCard label="Total Spent" value={money(completedSpend)} icon={<IndianRupee size={18} />} iconBg="#68D39120" iconColor={C.success} trend="Across all deals" />
