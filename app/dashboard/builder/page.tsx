...
function statusMeta(status?: string | null) {
  if (status === 'in_progress') return { label: 'In Progress', bg: '#00C2A820', color: C.blue, border: '#00C2A840' };
  ...
  if (status === 'completed') return { label: 'Completed', bg: '#22C55E20', color: C.success, border: '#22C55E40' };
  return { label: 'Open', bg: '#1B2540', color: C.gray, border: C.border };
}

function categoryMeta(category?: string | null) {
  const c = String(category || '').toLowerCase();
  if (c.includes('whatsapp')) return { label: 'WhatsApp', bg: '#25D36620', color: '#25D366' };
  if (c.includes('lead')) return { label: 'Lead Gen', bg: '#00C2A820', color: C.blue };
  if (c.includes('booking')) return { label: 'Booking', bg: '#F6AD5520', color: C.warning };
  if (c.includes('support')) return { label: 'Support', bg: '#5B5EF720', color: C.purple };
  if (c.includes('website')) return { label: 'Website', bg: '#22C55E20', color: C.success };
  return { label: category || 'Other', bg: '#1B2540', color: C.gray };
}
      ...
      <main className="dash-main">
        ...
        <div className="stats-grid">
          {loading ? Array.from({ length: 4 }).map((_, i) => <div key={i} className="stat-card"><Skeleton className="h-4 w-24" /><Skeleton className="mt-5 h-8 w-20" /></div>) : (
            <>
              <StatCard label="Total Earned" value={money(totalEarned)} icon={<IndianRupee size={18} />} iconBg="#22C55E20" iconColor={C.success} trend={`+${money(thisMonth)} this month`} />
              <StatCard label="Active Deals" value={deals.filter((d) => d.status === 'in_progress').length} icon={<Briefcase size={18} />} iconBg="#00C2A820" iconColor={C.blue} trend="In progress" />
              ...
              <StatCard label="Agents Listed" value={publishedCount} icon={<Package size={18} />} iconBg="#5B5EF720" iconColor={C.purple} trend={`${publishedCount}/${agentLimit >= 999 ? '∞' : agentLimit} based on plan`} />
            </>
          )}
        </div>
              ...
              {loading ? Array.from({ length: 3 }).map((_, r) => <tr key={r}>{Array.from({ length: 6 }).map((__, c) => <td key={c}><Skeleton className="h-4 w-24" /></td>)}</tr>) : agents.length ? agents.map((agent) => {
                ...
                return (
                  <tr key={agent.id}>
                    ...
                    <td><span className="badge" style={{ background: live ? '#22C55E20' : pending ? '#F6AD5520' : '#1B2540', color: live ? C.success : pending ? C.warning : C.gray, borderColor: live ? '#22C55E40' : pending ? '#F6AD5540' : C.border }}>{live ? 'Live ✓' : pending ? 'Under Review' : 'Draft'}</span></td>
